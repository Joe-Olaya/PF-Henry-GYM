...
const Pagination = () =>
{
   ...
   return (
      <div className="flex bg-neutral-800 rounded-lg font-[Poppins]">
         <button onClick={back} className="h-12 border-2 border-r-0 border-yellow-500
               px-4 rounded-l-lg hover:bg-neutral-900 hover:text-yellow-500">
         ...
         {
            pages.map((pg, i) => (
               <button key={i} onClick={() => setCur(pg.page)} className={`h-12 border-2 border-r-0 border-yellow-500
               w-12 ${cur === pg.page && 'bg-neutral-900 text-yellow-500'}`}>{pg.page}</button>
            ))
         }
         <button onClick={Next} className="h-12 border-2  border-yellow-500
               px-4 rounded-r-lg bg-neutral-900 hover:text-yellow-500">
